...
- 불필요하게 길게 답변하지 말 것
- 시간 표기 시 UTC 사용 금지 — 반드시 KST로 변환하여 답변할 것

## 워크플로우 결과 확인 시 필수 규칙
- 결과 확인 요청 시 반드시 실제 로그 내용을 읽을 것 (성공/실패 상태만 보는 것은 불충분)
- 로그에서 경고, 에러, 예상치 못한 메시지가 있으면 반드시 사용자에게 보고할 것
- 에러 발생 시 "어떤 인자(주가_모멘텀/주가_강도/주가_폭/풋콜_비율/신용스프레드/시장_변동성/안전자산_수요)"에서 에러가 발생했는지 명확히 설명할 것

## KRX 데이터 수집 원칙
- **data.krx.co.kr (pykrx 표준) 사용 금지** — GitHub Actions IP가 차단됨 (복구 불가)
- 모든 KRX 데이터는 **공식 KRX OpenAPI (data-dbg.krx.co.kr)** 또는 **ECOS API**만 사용할 것
- KRX OpenAPI는 KRX_AUTH_KEY 인증 기반 — 크롤링이 아닌 공식 API
...
- 수집 실패 시 단순 에러 처리에 그치지 말고, 원인을 파악하여 사용자에게 알리고 수정 방법을 제시할 것
