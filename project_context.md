...
13. **업데이트_시각 CSV에 미저장** (수정 완료): `dropna(subset=existing)`의 기본값 `how="any"`로 인해 인자 중 하나라도 NaN이면 마지막 행 삭제 → `how="all"`로 변경하여 일부 인자 NaN이어도 행 보존. 화면(index.html)에서는 NaN 인자를 "—"으로 표시하는 로직이 이미 있음. **월요일(2026-06-23) 워크플로우 실행 후 반영 예정.**

## 업데이트_시각 표시 로직
- Python (`k_fear_greed_index.py:114-115`): 마지막 행에 `cfg.base_dt.strftime("%H:%M")` 설정
- HTML (`index.html:748-750`): `gaugeUpdated` 요소에 `(업데이트: HH:MM KST)` 표시
- 기존 문제: `dropna(how="any")`가 NaN 인자 포함 행을 삭제 → 업데이트 시각이 CSV에 저장 안 됨
- 수정: `dropna(how="all")`로 변경 — 모든 인자가 NaN인 행만 제거
...
- **사용자가 말하는 문제를 정확히 파악하지 않고 엉뚱한 수정**: 화면 표시 문제를 물었는데 CSV 로직을 먼저 수정하고, 되돌리고, 다시 수정하는 혼선 발생. 사용자의 의도를 먼저 정확히 파악할 것
- **앞으로 지켜야 할 것**:
  ...
  4. 사용자가 문제를 말하면, 수정하기 전에 원인을 정확히 설명하고 동의를 받을 것
